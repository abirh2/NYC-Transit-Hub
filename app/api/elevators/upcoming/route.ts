/**
 * Upcoming Elevator/Escalator Outages API
 * GET /api/elevators/upcoming
 * 
 * Returns planned/upcoming elevator and escalator outages.
 * Fetches from MTA upcoming outages feed.
 * 
 * Query Parameters:
...
import { fetchUpcomingOutages, filterOutages, getOutageSummary } from "@/lib/mta";
...
export async function GET(
  ...
  try {
    // Fetch upcoming outages from MTA
    let outages = await fetchUpcomingOutages();
    ...
    console.error("Error fetching upcoming elevator outages:", error);
    
    return NextResponse.json({
      success: false,
      data: null,
      error: error instanceof Error ? error.message : "Failed to fetch upcoming elevator outages",
      timestamp: new Date().toISOString(),
    }, { status: 500 });
  }
}
