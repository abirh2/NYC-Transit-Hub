...
export async function GET(
  ...
  try {
    ...
    if (stationId) {
      const station = getLirrStationById(stationId);
    ...
    // If near parameter provided - find nearby stations
      ...
      const allStations = getAllLirrStations();
    ...
    // If search query provided
      ...
      const results = searchLirrStations(search, limit);
    ...
    console.error("Error fetching LIRR stations:", error);

    return NextResponse.json(
      {
        success: false,
        data: null,
        error: error instanceof Error ? error.message : "Failed to fetch LIRR stations",
        timestamp: new Date().toISOString(),
      },
      { status: 500 }
    );
